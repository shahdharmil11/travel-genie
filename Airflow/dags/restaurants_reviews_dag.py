...
STAGE_NAME = "RESTAURANT_REVIEWS_STAGE"
REVIEWS_TABLE = "RESTAURANT_REVIEWS"
REVIEW_URLS = []
...
# path = os.getcwd()
output_file = "/opt/airflow/dags/sample_restaurant_reviews_output.csv"
print(output_file)
...
# """
copy_into_sql = f"""
COPY INTO TRAVEL_GENIE.RAW.{REVIEWS_TABLE} (
    ID, LOCATIONID, CITY, COUNTRY, POSTALCODE, STATE, STREET1, STREET2, LATITUDE, LONGITUDE, 
    NAME, PLACE_RATING, PUBLISHEDDATE, PUBLISHEDPLATFORM, RATING, 
    SUBRATING_NAME_1, SUBRATING_VALUE_1, SUBRATING_NAME_2, SUBRATING_VALUE_2,
    SUBRATING_NAME_3, SUBRATING_VALUE_3, SUBRATING_NAME_4, SUBRATING_VALUE_4,
    TEXT, TITLE, TRAVELDATE, TRIP_TYPE, URL, USER_LINK, USER_NAME, USERID, USERNAME
)
FROM @{STAGE_NAME}
...
def fetch_restaurant_reviews_data(**context):
...
def save_reviews_to_csv(**context):
    ...
    with open(output_file, 'w') as file:
        ...
        for item in data:
            ...
            subrating_names = [sr.get("name") for sr in subratings[:4]]
            subrating_values = [sr.get("value") for sr in subratings[:4]]
            ...
            row = {
                ...
                "SUBRATING_NAME_1": subrating_names[0] if len(subrating_names) > 0 else None,
                "SUBRATING_VALUE_1": subrating_values[0] if len(subrating_values) > 0 else None,
                "SUBRATING_NAME_2": subrating_names[1] if len(subrating_names) > 1 else None,
                "SUBRATING_VALUE_2": subrating_values[1] if len(subrating_values) > 1 else None,
                "SUBRATING_NAME_3": subrating_names[2] if len(subrating_names) > 2 else None,
                "SUBRATING_VALUE_3": subrating_values[2] if len(subrating_values) > 2 else None,
                "SUBRATING_NAME_4": subrating_names[3] if len(subrating_names) > 3 else None,
                "SUBRATING_VALUE_4": subrating_values[3] if len(subrating_values) > 3 else None,
                "TEXT": item.get("text"),
...
dag = DAG(
    'create_restaurant_stages_tables_snowflake',
...
dag1 = DAG(
    'tripadvisor_restaurant_reviews_etl',
    default_args=default_args,
    description='ETL pipeline for Tripadvisor restaurant reviews data using Apify Actor',
    schedule_interval='@daily',
    catchup=False
)
# # Tasks
fetch_restaurant_reviews_data_task = PythonOperator(
    task_id='fetch_restaurant_reviews_data_task',
    python_callable=fetch_restaurant_reviews_data,
...
create_stage = SnowflakeOperator(
    task_id='create_restaurant_stage',
...
create_reviews_table = SnowflakeOperator(
    task_id='create_restaurant_reviews_table',
...
fetch_restaurant_reviews_data_task >> save_to_csv >> load_data
# load_data
